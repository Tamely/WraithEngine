...
glm::vec4 ParseHexColor(std::string_view Hex) {
  ...
  return {ParseChannel(Hex[1], Hex[2]) / 255.0f,
          ParseChannel(Hex[3], Hex[4]) / 255.0f,
          ParseChannel(Hex[5], Hex[6]) / 255.0f, 1.0f};
}
...
const EditorTransformDetails *GetEffectiveTransform(const EditorObjectDetails &Details) {
  if (Details.WorldTransform.has_value()) return &*Details.WorldTransform;
  if (Details.Transform.has_value()) return &*Details.Transform;
  return nullptr;
}
...
HeadlessOverlayModule::HeadlessOverlayModule(EditorSession &Session)
    : m_Session(Session) {}
...
void HeadlessOverlayModule::SetPresenceMarkerMeshForTesting(MeshRef Mesh) {
  m_PresenceMarkerMesh = std::move(Mesh);
}

void HeadlessOverlayModule::SetColliderMeshesForTesting(MeshRef BoxMesh,
                                                        MeshRef SphereMesh) {
  m_ColliderBoxMesh = std::move(BoxMesh);
  m_ColliderSphereMesh = std::move(SphereMesh);
}

void HeadlessOverlayModule::SetGizmoHoveredAxis(SessionUserId User, int Axis) {
  std::lock_guard Lock(m_GizmoHoverMutex);
  m_GizmoHoveredAxisByUser[User.Value] = Axis;
}

int HeadlessOverlayModule::GetGizmoHoveredAxis(SessionUserId User) const {
  std::lock_guard Lock(m_GizmoHoverMutex);
  const auto It = m_GizmoHoveredAxisByUser.find(User.Value);
  return It != m_GizmoHoveredAxisByUser.end() ? It->second : -1;
}

void HeadlessOverlayModule::SetGizmoMode(SessionUserId User, GizmoMode Mode) {
  std::lock_guard Lock(m_GizmoModeMutex);
  m_GizmoModeByUser[User.Value] = Mode;
}

GizmoMode HeadlessOverlayModule::GetGizmoMode(SessionUserId User) const {
  std::lock_guard Lock(m_GizmoModeMutex);
  const auto It = m_GizmoModeByUser.find(User.Value);
  return It != m_GizmoModeByUser.end() ? It->second : GizmoMode::Translate;
}

std::vector<LightBillboardOverlay> HeadlessOverlayModule::BuildLightBillboards() const {
...
HeadlessOverlayModule::BuildColliderOverlaySubmissions() const {
  ...
  for (const auto &[Id, Details] : m_Session.GetState().Scene.ObjectDetailsById) {
    ...
    if (Physics.ColliderType == EditorPhysicsColliderType::Box) {
      if (m_ColliderBoxMesh == nullptr) continue;
      ...
      if (m_ColliderSphereMesh == nullptr) continue;
      ColliderMesh = m_ColliderSphereMesh;
      ColliderTransform *= glm::scale(
          glm::mat4(1.0f), glm::vec3(Physics.SphereRadius * ColliderOverlayScale));
    } else {
      continue;
    }
    Result.push_back({
        ...
        for (int Z = -1; Z <= 1; Z += 2) {
          const glm::vec3 LocalOffset =
              glm::vec3(static_cast<float>(X), static_cast<float>(Y),
                        static_cast<float>(Z)) * HalfExtents;
...
std::vector<RenderMeshSubmission>
HeadlessOverlayModule::BuildPresenceOverlaySubmissions(SessionUserId RenderUser) const {
  std::vector<RenderMeshSubmission> Result;
  ...
  for (const EditorParticipant &Participant : Participants) {
    if (Participant.User.Value == 1 || Participant.User.Value == RenderUser.Value ||
...
HeadlessOverlayModule::GetOrCreatePresenceMaterial(SessionUserId User) const {
  ...
  Texture->Width = 1;
  Texture->Height = 1;
  Texture->Pixels = {static_cast<uint8_t>(Color.r * 255.0f),
