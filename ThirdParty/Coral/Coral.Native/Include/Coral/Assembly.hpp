...
namespace Coral {
	...
	public:
		...
		/// Re-populates the managed s_CachedTypes table for this assembly.
		///
		/// After UnloadAssemblyLoadContext the global managed UniqueIdList<Type> is cleared,
		/// making all previously-issued TypeIds invalid on the managed side even though the
		/// C++ m_LocalTypeNameCache still holds them.  Calling this method re-drives
		/// GetAssemblyTypes so the managed side re-registers the same Type objects under the
		/// same stable RuntimeHelpers.GetHashCode IDs, keeping C++ TypeId handles valid.
