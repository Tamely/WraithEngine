...
namespace Coral {
	...
	void ManagedAssembly::RefreshTypeCache()
	{
		// Re-drive GetAssemblyTypes so that the managed UniqueIdList<Type> re-registers
		// the same Type objects. After UnloadAssemblyLoadContext the managed side clears
		// its global type cache, making stored TypeIds stale.  Calling GetAssemblyTypes
		// again causes the managed side to re-insert the same Type objects under their
		// stable RuntimeHelpers.GetHashCode IDs, restoring C++/managed TypeId consistency.
		int32_t typeCount = 0;
		s_ManagedFunctions.GetAssemblyTypesFptr(m_OwnerContextId, m_AssemblyId, nullptr, &typeCount);
		if (typeCount <= 0)
			return;
		std::vector<TypeId> typeIds(static_cast<size_t>(typeCount));
		s_ManagedFunctions.GetAssemblyTypesFptr(m_OwnerContextId, m_AssemblyId, typeIds.data(), &typeCount);
	}
