...
public static class AssemblyLoader
{
	...
	internal static Assembly? ResolveAssembly(AssemblyLoadContext? InAssemblyLoadContext, AssemblyName InAssemblyName)
	{
		try
		{
			...
			// Before probing directories, check if another ALC (e.g. the engine ALC) already
			// has this assembly loaded. This prevents a second copy of WraithEngine.Managed from
			// being loaded into the UserScripts ALC, which would give it null internal-call
			// function pointers.
			foreach (var otherCacheEntry in s_AssemblyCache)
			{
				if (otherCacheEntry.Key == alcId)
					continue;
				if (otherCacheEntry.Value.TryGetValue(assemblyId, out var sharedAssembly) && sharedAssembly != null)
				{
					LogMessage($"[AssemblyLoader] Resolved {InAssemblyName.Name} from sibling ALC cache", MessageLevel.Trace);
