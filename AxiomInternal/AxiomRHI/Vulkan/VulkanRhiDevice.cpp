...
std::unique_ptr<IRHIFence> VulkanRhiDevice::CreateFence() { return nullptr; }
...
void VulkanRhiDevice::PrepareSceneFrame(RenderScene &Scene) {
  m_ActiveScene = &Scene;
  ResetPreparedSceneState();
  m_QueuedScenePasses.clear();
...
const VisibleSubmissionList &VulkanRhiDevice::GetVisibleSubmissions() const {
  return m_PreparedSceneState.VisibleSubmissions;
}

void VulkanRhiDevice::PrepareGraphicsMaterialDescriptors() {
#if !defined(NDEBUG)
...
void VulkanRhiDevice::RecordDepthPrepass() {
  QueueScenePass(ScenePassPrimitive::DepthPrepass);
}

void VulkanRhiDevice::BuildHzb() { QueueScenePass(ScenePassPrimitive::Hzb); }

void VulkanRhiDevice::RecordOpaqueForward() {
  QueueScenePass(ScenePassPrimitive::OpaqueForward);
}

void VulkanRhiDevice::RecordTranslucentForward() {
  QueueScenePass(ScenePassPrimitive::TranslucentForward);
}

void VulkanRhiDevice::RecordComputeMeshPath() {
  QueueScenePass(ScenePassPrimitive::ComputeMeshPath);
}

void VulkanRhiDevice::RecordBackground() {
  QueueScenePass(ScenePassPrimitive::Background);
}

void VulkanRhiDevice::FinalizeSceneFrame() {
  m_PreparedSceneState.HasQueuedFinalize = true;
}

void VulkanRhiDevice::RenderFallbackBackground(RenderScene &Scene) {
  m_ActiveScene = &Scene;
...
RendererFrameStats &VulkanRhiDevice::AccessFrameStats() {
  return m_DrawSubmissionSystem.AccessFrameStats();
}

const RendererFrameStats &VulkanRhiDevice::GetFrameStats() const {
  return m_DrawSubmissionSystem.GetFrameStats();
}

void VulkanRhiDevice::RenderImGui() {
  m_DrawSubmissionSystem.RenderImGui(m_StopRendering, m_ViewMode);
}

void VulkanRhiDevice::Draw() {
...
void VulkanRhiDevice::RecordPreparedScenePasses(
...
void VulkanRhiDevice::DrawBackgroundPass(VkCommandBuffer CommandBuffer,
                                               RenderScene *Scene) {
...
void VulkanRhiDevice::BuildHzbPass(VkCommandBuffer CommandBuffer,
...
void VulkanRhiDevice::QueueScenePass(ScenePassPrimitive Pass) {
  m_QueuedScenePasses.push_back(Pass);
}

void VulkanRhiDevice::ResetPreparedSceneState() {
  m_PreparedSceneState = {};
  m_PreparedSceneState.VisibleSubmissions.Clear();
}

glm::vec3 VulkanRhiDevice::ComputeWorldCenter(
...
CameraFrameUniform
VulkanRhiDevice::BuildCameraData(const RenderScene &Scene,
...
void VulkanRhiDevice::UpdateComputeFrameDescriptors(
...
void VulkanRhiDevice::UpdateDepthFrameDescriptors(
    const MeshFrameResources &Frame) const {
  VkDescriptorBufferInfo CameraBufferInfo =
      VkInit::BufferInfo(Frame.CameraBuffer.Buffer, 0, Frame.CameraBuffer.Size);
  VkDescriptorBufferInfo MutableCameraBufferInfo = CameraBufferInfo;
...
void VulkanRhiDevice::UpdateGraphicsFrameDescriptors(
    const MeshFrameResources &Frame) const {
  VkDescriptorBufferInfo CameraBufferInfo =
      VkInit::BufferInfo(Frame.CameraBuffer.Buffer, 0, Frame.CameraBuffer.Size);
  VkDescriptorBufferInfo MutableCameraBufferInfo = CameraBufferInfo;
...
void VulkanRhiDevice::RecordDepthPrepassPass(
...
void VulkanRhiDevice::RecordComputeMeshPathPass(
    VkCommandBuffer CommandBuffer, const MeshFrameResources &Frame) const {
...
void VulkanRhiDevice::RecordOpaqueForwardPass(
...
void VulkanRhiDevice::RecordTranslucentForwardPass(
    VkCommandBuffer CommandBuffer, const MeshFrameResources &Frame) {
...
void VulkanRhiDevice::EnsureDrawImageLayout(
...
void VulkanRhiDevice::EnsureRasterDepthLayout(
    VkCommandBuffer CommandBuffer, VkImageLayout DesiredLayout) {
...
void VulkanRhiDevice::BindMeshBuffers(VkCommandBuffer CommandBuffer,
...
VulkanRhiDevice::GetSubmission(uint32_t SubmissionIndex) const {
  assert(m_PreparedSceneState.Scene != nullptr);
  return m_PreparedSceneState.Scene->Submissions[SubmissionIndex];
}

VulkanMesh *
VulkanRhiDevice::ResolveVisibleMesh(const VisibleSubmission &Visible) const {
...
VkExtent2D VulkanRhiDevice::GetDrawExtent2D() const {
...
void VulkanRhiDevice::SetViewMode(RendererViewMode ViewMode) {
  m_ViewMode = ViewMode;
}

void VulkanRhiDevice::SetViewportFrameUser(SessionUserId User) {
  m_ViewportFrameUser = User;
}

void VulkanRhiDevice::SetViewportFrameOutput(
    IViewportFrameOutput *FrameOutput) {
  m_FrameOutput = FrameOutput;
}

std::optional<CapturedFrame> VulkanRhiDevice::ConsumeCapturedFrame() {
